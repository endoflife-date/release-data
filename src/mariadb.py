import urllib.request
import json

PRODUCT = 'mariadb'
API = 'https://downloads.mariadb.org/rest-api/mariadb/'

HISTORICAL = {
  "10.2.43": "2022-02-12",
  "10.2.42": "2022-02-08",
  "10.2.41": "2021-11-08",
  "10.2.40": "2021-08-05",
  "10.2.39": "2021-06-08",
  "10.2.38": "2021-05-04",
  "10.2.37": "2021-02-18",
  "10.2.36": "2020-11-09",
  "10.2.35": "2020-10-31",
  "10.2.34": "2020-10-05",
  "10.2.33": "2020-08-06",
  "10.2.32": "2020-05-08",
  "10.2.31": "2020-01-26",
  "10.2.30": "2019-12-05",
  "10.2.29": "2019-11-06",
  "10.2.28": "2019-10-31",
  "10.2.27": "2019-09-08",
  "10.2.26": "2019-07-27",
  "10.2.25": "2019-06-14",
  "10.2.24": "2019-05-08",
  "10.2.23": "2019-03-23",
  "10.2.22": "2019-02-10",
  "10.2.21": "2018-12-30",
  "10.2.20": "2018-12-21",
  "10.2.19": "2018-11-12",
  "10.2.18": "2018-09-25",
  "10.2.17": "2018-08-12",
  "10.2.16": "2018-06-25",
  "10.2.15": "2018-05-16",
  "10.2.14": "2018-03-26",
  "10.2.13": "2018-02-12",
  "10.2.12": "2018-01-03",
  "10.2.11": "2017-11-27",
  "10.2.10": "2017-10-30",
  "10.2.9": "2017-09-25",
  "10.2.8": "2017-08-17",
  "10.2.7": "2017-07-10",
  "10.2.6": "2017-05-15",
  "10.1.48": "2020-10-30",
  "10.1.47": "2020-10-05",
  "10.1.46": "2020-08-06",
  "10.1.45": "2020-05-08",
  "10.1.44": "2020-01-25",
  "10.1.43": "2019-11-06",
  "10.1.42": "2019-10-31",
  "10.1.41": "2019-07-26",
  "10.1.40": "2019-05-07",
  "10.1.39": "2019-04-30",
  "10.1.38": "2019-02-04",
  "10.1.37": "2018-10-31",
  "10.1.36": "2018-09-07",
  "10.1.35": "2018-08-04",
  "10.1.34": "2018-06-16",
  "10.1.33": "2018-05-08",
  "10.1.32": "2018-03-26",
  "10.1.31": "2018-02-04",
  "10.1.30": "2017-12-21",
  "10.1.29": "2017-11-13",
  "10.1.28": "2017-09-27",
  "10.1.27": "2017-09-22",
  "10.1.26": "2017-08-09",
  "10.1.25": "2017-07-01",
  "10.1.24": "2017-05-29",
  "10.1.23": "2017-05-02",
  "10.1.22": "2017-03-11",
  "10.1.21": "2017-01-17",
  "10.1.20": "2016-12-14",
  "10.1.19": "2016-11-04",
  "10.1.18": "2016-09-29",
  "10.0.38": "2019-01-29",
  "10.0.37": "2018-10-30",
  "10.0.36": "2018-07-31",
  "10.0.35": "2018-05-01",
  "10.0.34": "2018-01-27",
  "10.0.33": "2017-10-27",
  "10.0.32": "2017-08-04",
  "10.0.31": "2017-05-20",
  "10.0.30": "2017-03-06",
  "10.0.29": "2017-01-12",
  "10.0.28": "2016-10-27",
  "10.0.27": "2016-08-24",
  "10.0.26": "2016-06-23",
  "10.0.25": "2016-04-28",
  "10.0.24": "2016-02-17",
  "10.0.23": "2015-12-16",
  "10.0.22": "2015-10-28",
  "10.0.21": "2015-08-05",
  "10.0.20": "2015-06-17",
  "10.0.19": "2015-05-08",
  "10.0.18": "2015-05-06",
  "10.0.17": "2015-02-25",
  "10.0.16": "2015-01-25",
  "10.0.15": "2014-11-21",
  "10.0.14": "2014-09-24",
  "10.0.13": "2014-08-08",
  "10.0.12": "2014-06-12",
  "5.5.68": "2020-05-06",
  "5.5.67": "2020-01-18",
  "5.5.66": "2019-10-30",
  "5.5.65": "2019-07-25",
  "5.5.64": "2019-04-26",
  "5.5.63": "2019-01-27",
  "5.5.62": "2018-10-24",
  "5.5.61": "2018-07-30",
  "5.5.60": "2018-04-20",
  "5.5.59": "2018-01-18",
  "5.5.58": "2017-10-17",
  "5.5.57": "2017-07-18",
  "5.5.56": "2017-04-30",
  "5.5.55": "2017-04-11",
  "5.5.54": "2016-12-22",
  "5.5.53": "2016-10-14",
  "5.5.52": "2016-09-12",
  "5.5.51": "2016-08-09",
  "5.5.50": "2016-06-16",
  "5.5.49": "2016-04-20",
  "5.5.48": "2016-02-10",
  "5.5.47": "2015-12-09",
  "5.5.46": "2015-10-09",
  "5.5.45": "2015-08-04",
  "5.5.44": "2015-06-09",
  "5.5.43": "2015-04-29",
  "5.5.42": "2015-02-13",
  "5.5.41": "2014-12-19",
  "5.5.40": "2014-10-08",
  "5.5.39": "2014-08-03",
  "5.5.38": "2014-06-05",
  "5.5.37": "2014-04-15",
  "5.5.36": "2014-02-22",
  "5.5.35": "2014-01-28",
  "5.5.34": "2013-11-18",
  "5.5.33": "2013-09-16",
  "5.5.32": "2013-07-17",
  "5.5.31": "2013-05-21",
  "5.5.30": "2013-03-11",
  "5.5.29": "2013-01-29",
  "5.5.29": "2013-01-29",
  "5.5.30": "2013-03-11",
  "5.5.31": "2013-05-21",
  "5.5.32": "2013-07-17",
  "5.5.33": "2013-09-16",
  "5.5.34": "2013-11-18",
  "5.5.35": "2014-01-28",
  "5.5.36": "2014-02-22",
  "5.5.37": "2014-04-15",
  "5.5.38": "2014-06-05",
  "5.5.39": "2014-08-03",
  "5.5.40": "2014-10-08",
  "5.5.41": "2014-12-19",
  "5.5.42": "2015-02-13",
  "5.5.43": "2015-04-29",
  "5.5.44": "2015-06-09",
  "5.5.45": "2015-08-04",
  "5.5.46": "2015-10-09",
  "5.5.47": "2015-12-09",
  "5.5.48": "2016-02-10",
  "5.5.49": "2016-04-20",
  "5.5.50": "2016-06-16",
  "5.5.51": "2016-08-09",
  "5.5.52": "2016-09-12",
  "5.5.53": "2016-10-14",
  "5.5.54": "2016-12-22",
  "5.5.55": "2017-04-11",
  "5.5.56": "2017-04-30",
  "5.5.57": "2017-07-18",
  "5.5.58": "2017-10-17",
  "5.5.59": "2018-01-18",
  "5.5.60": "2018-04-20",
  "5.5.61": "2018-07-30",
  "5.5.62": "2018-10-24",
  "5.5.63": "2019-01-27",
  "5.5.64": "2019-04-26",
  "5.5.65": "2019-07-25",
  "5.5.66": "2019-10-30",
  "5.5.67": "2020-01-18",
  "5.5.68": "2020-05-06"
}

def fetch_cycles():
    with urllib.request.urlopen(API) as response:
        data = json.loads(response.read().decode("utf-8"))
        return [ x['release_id'] for x in filter(lambda r : r['release_status'] == 'Stable', data['major_releases']) ]
    return []

def fetch_releases(cycles):
    releases = {}

    for cycle in cycles:
        url = API + cycle
        with urllib.request.urlopen(url) as response:
            data = json.loads(response.read().decode("utf-8"))
            for release in filter(lambda r : r['release_status'] == 'Stable', data['releases'].values()):
                releases[release['release_id']] = release['date_of_release']
    return releases

def print_releases(releases):
    for version, date in releases.items():
        print(f"{version} : {date}")

def main():
    print(f"::group::{PRODUCT}")

    cycles = fetch_cycles()
    releases = fetch_releases(cycles)
    print_releases(releases)

    print("::endgroup::")
    releases.update(HISTORICAL)

    with open(f"releases/{PRODUCT}.json", "w") as f:
        f.write(json.dumps(releases, indent=2))

if __name__ == '__main__':
    main()

